...
const AlarmSettingsroom1 = () => {
  ...
  useEffect(() => {
    // ดึงค่าขีดจำกัดอุณหภูมิและเวลาปัจจุบันจากเซิร์ฟเวอร์เมื่อคอมโพเนนต์ถูกโหลด
    fetch('http://localhost:4000/get-threshold')
    ...
    fetch('http://localhost:4050/get-threshold')
      .then(response => response.json())
      .then(data => {
    ...
    return () => clearInterval(timer); // ล้างข้อมูลเมื่อคอมโพเนนต์ถูกยกเลิกการติดตั้ง
  }, []);
  ...
  const handleSaveSettings = () => {
    ...
    fetch('http://localhost:4000/set-threshold', {
      method: 'POST',
      headers: {
      ...
      .then(response => response.json())
      .then(data => {
        setCurrentThreshold(threshold); // อัปเดตสถานะขีดจำกัดปัจจุบัน
      })
      .catch(error => {
  ...
  const handleSaveTimeSettings = () => {
    ...
    fetch('http://localhost:4050/set-threshold', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      ...
      .then(response => response.json())
      .then(data => {
        setCurrentTimeThreshold(timeThreshold); // อัปเดตสถานะขีดจำกัดเวลาปัจจุบัน
      })
      .catch(error => {
...
export default AlarmSettingsroom1;
